...
import { FaEye, FaCheck, FaSync, FaSort, FaSortUp, FaSortDown } from "react-icons/fa";

export default function Orders() {
  const [orders, setOrders] = useState([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("PENDING"); // Mặc định hiển thị PENDING
  const [sortBy, setSortBy] = useState("date-desc"); // Mặc định sắp xếp theo ngày mới nhất
  ...
  const filteredOrders = orders
    .filter((order) => {
      const matchSearch =
        ...
        (order.customerTrading?.receiverName &&
          order.customerTrading.receiverName.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (order.customerTrading?.receiverPhone &&
          order.customerTrading.receiverPhone.includes(searchTerm));

      const matchStatus = statusFilter === "all" || order.statusOrder === statusFilter;
    ...
    .sort((a, b) => {
      switch (sortBy) {
        ...
        case "price-desc":
          return (b.customerTrading?.totalAmount || 0) - (a.customerTrading?.totalAmount || 0);
        case "price-asc":
          return (a.customerTrading?.totalAmount || 0) - (b.customerTrading?.totalAmount || 0);
        default:
          return 0;
      }
    });
  ...
  const handleConfirmOrder = async (orderId) => {
    ...
    const confirmMessage = isCashPayment
      ? "Confirm this order?\nAn invoice will be automatically generated (Cash on Delivery)."
      : "Confirm this order?\n(Payment already received — no invoice will be created)";
    ...
    try {
      ...
      // Step 2: Create invoice only for CASH
      if (isCashPayment) {
        try {
          await handleCreateInvoice(orderId);
          alert("Order confirmed successfully and invoice has been created!");
        } catch (invoiceError) {
          console.error("Invoice creation failed:", invoiceError);
          alert(
            "Order confirmed successfully!\n" +
              "Warning: Automatic invoice creation failed. Please create it manually in the Invoices section."
          );
        }
      } else {
        alert("Order confirmed successfully!\n(Customer has already paid — no invoice created)");
      }
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 text-lg">Manage and track customer orders</p>
            </div>
            {/* Refresh Button - Small & Corner */}
            <button
              onClick={loadOrders}
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full max-h-[92vh] overflow-y-auto">
            ...
            <div className="sticky top-0 bg-gradient-to-r from-gray-800 to-gray-700 px-8 py-6 flex justify-between items-center rounded-t-xl z-10">
              <div>
                ...
                <p className="text-gray-300 text-sm mt-1">Complete order information</p>
              </div>
              <button
            ...
            <div className="p-8 space-y-6">
              {/* 1. Order Information - Most Important */}
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-red-600">
                      {formatPrice(selectedOrder.customerTrading?.totalAmount || 0)}
                    </p>
                  </div>
                  {selectedOrder.note && (
                    <div className="col-span-3">
                      <p className="text-sm text-gray-600 font-semibold mb-1">Note</p>
              ...
              {/* 2. Receiver Information (Customer Trading) */}
              <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm text-gray-600 font-semibold mb-1">Receiver Name</p>
                    <p className="text-lg font-bold text-gray-900">
              ...
              {/* 3. Account Information (Order Placer) */}
                    ...
                    <table className="w-full">
                      <thead className="bg-gray-800 text-white">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-right text-sm font-bold">Total Price</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-200">
